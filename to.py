...
df = pd.read_excel('Tes Excel Admin Klinik.xlsx', skiprows=2, nrows=5, usecols="C:G", names=['Nama Pasien', 'Kelas', 'Penanganan', 'Tanggal Masuk', 'Tanggal Keluar'])
# df = df.dropna()
print(df)
...
df['Lama Tinggal2'] = df['Tanggal Keluar'] - df['Tanggal Masuk']
print(df)
def inap(x,y):
    if x == 'A':
        return 300000*y
...
print(tabel_akhir)
hasil = tabel_akhir.set_index('Kelas')
print(hasil)

hasil = tabel_akhir.reset_index()
print(hasil)
