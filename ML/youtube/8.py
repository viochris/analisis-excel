...
test = pd.read_csv('ML/youtube/test.csv')
train = pd.read_csv('ML/youtube/train.csv')
...
# train.columns = train.columns.str.replace('Embarked_', '')
# test.columns = test.columns.str.replace('Embarked_', '')
...
# test['title'] = test['Name'].str.extract(' ([A-Za-z]+.)\ ', expand=False) #Jika ingin diberi titik, misal MR., MRS., DLL
test['title'] = test['title'].map(title_mapping)

# train.columns = train.columns.str.replace('Embarked_', ' ') #error, harusnya print(train[' C']) bukan print(train['C'])
# train.columns = train.columns.str.replace('Embarked_', ' ').str.strip() #bisa
# test.columns = test.columns.str.replace('Embarked_', '')
...
hasil.to_csv('Praktek/Youtube/submission_more_true3.csv', index=False)
