...
print(df)
tabel1 = df.groupby(['category'])[['after_discount', 'qty_ordered']].sum().reset_index()
print(tabel1)

tabel2 = df.drop_duplicates('customer_id')
print('hasil:', tabel2.shape)

tabel2 = tabel2.groupby(['category'])[['customer_id', 'id_x']].nunique().reset_index()
print(tabel2)
print(tabel2['customer_id'].sum())

tabel_akhir = pd.merge(tabel1, tabel2, on=['category'], how='inner')
