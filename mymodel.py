...
fig, ax = plt.subplots()
ax.bar(x - width / 2, tabel_mix['weekend'], width, label='weekend')
ax.bar(x + width / 2, tabel_mix['weekdays'], width, label='weekdays')
ax.set_xticks(x)
ax.set_xticklabels(tabel_mix['month'])
ax.set_xlabel('Bulan')
ax.set_ylabel('Penjualan')
ax.set_title('Diagram Penjualan Weekdays vs Weekend In 2022')
ax.legend()
ax.grid()
st.title('Weekdays vs Weekend')
st.write('This is the difference on weekdays vs weekends in 2022.')
st.pyplot(fig)