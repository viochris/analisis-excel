...
df = pd.read_excel('Tes Excel Shopee Data Analyst.xlsx',nrows=15, sheet_name='Test', usecols='B')
...
print(pivot)
print(pivot[pivot.index.get_level_values('CUSTOMER NAME') == 'Progo Depstore'])

print('\n\n')

pivot = df_asal.groupby(['AREA', 'CUSTOMER NAME'])[['BOX QTY', 'TOTAL WEIGHT']].sum().reset_index().sort_values('AREA', ascending=True)
print(pivot)
print(pivot[pivot['CUSTOMER NAME'] == 'Progo Depstore'])