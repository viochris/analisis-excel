...
print(tabel_apriori)
print()
product_association = association_rules(tabel_apriori, metric='confidence', min_threshold=0.5).sort_values('confidence', ascending=False).reset_index(drop=True)
print(product_association)
print()
product_association['antecedents'] = product_association['antecedents'].apply(lambda x: ' '.join(list(x)))
product_association['consequents'] = product_association['consequents'].apply(lambda x: ' '.join(list(x)))
product_association.to_excel('hasil akhir.xlsx', index=False)